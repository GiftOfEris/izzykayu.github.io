...
---
## NATIONAL SNAPSHOT: OPIOID DEATHS
...
Changeplot <-fullopioiddeathset_2013_2015 %>% 
  ...
  labs(x="", y="Opioid Death Rate Change", title="Percentage Change in Opioid Death Rates from 2013 to 2014") +
...
deathrate2013.plot <-fullopioiddeathset_2013_2015 %>% 
  ggplot(aes(state, Rate.2013, col=region))  + geom_point(alpha=0.42,aes(size=Rate.2014)) + coord_flip() +
  labs(x="", y="Opioid Death Rate", subtitle="Age and Population Adjusted 2013 Death Rates") +
        theme(axis.text = element_text(hjust = 1, size=6.7),
              axis.title = element_text(size=9), legend.position = "none") + scale_color_manual(values=cols)  +   # Draw points
...
deathrate2014.plot <-fullopioiddeathset_2013_2015 %>% 
  ggplot(aes(state, Rate.2014, col=region)) + geom_point(alpha=0.42, aes(size=Rate.2014)) + 
  labs(x="", y="Opioid Death Rate", subtitle="Age and Population Adjusted 2014 Death Rates") +
        theme(axis.text = element_text(hjust = 1, size=6.7), axis.title = element_text(size=9),
...
op2 <- longset %>% gather(Rate.2013,Rate.2014,Rate.2015, key="yr", value="DeathRate") %>% 
  ggplot(aes(DeathRate, Deaths, col=Year,size=DeathRate)) + geom_jitter(alpha=0.2) + 
  geom_point(alpha=0.6, pch=21) + scale_color_manual(values=c('#69D2E7','#C7F464','#CC4E64')) + 
  labs(subtitle="Opioid Death Rate vs Opioid Deaths") + theme(axis.text = element_text(hjust = 1, size=6.7), axis.title = element_text(size=9))
jpeg('op2op1.jpg')
multiplot(op2, op1, cols=2)
dev.off()
```
```{r}
...
DF <- fullopioiddeathset_2013_2015 %>% 
  right_join(longset,by = c("state", "Range.2015", "Rate.2015", "name", "region",
                            "division", "capital", "area", "population", "Rate.2014", "Range.2014",
                            "Rate.2013", "Range.2013", "Change", "Significant"))
...
DFDF %>% ggplot(aes(numbers, Percentage)) + 
  geom_point(aes(size=abs((dffff$Percentage -.0001)*2901),col=ifelse(Percentage > 0, "grey", "maroon")),
  ...
  geom_hline(yintercept = c(0), alpha=0.7) + geom_point(aes(size=abs((Percentage-.0001)*2999), 
                                                            col=ifelse(Percentage > 0, "grey", "maroon")), pch=21, alpha=0.9) + theme(legend.position = "none", axis.ticks.x = element_blank(), axis.text.x = element_blank()) + 
  labs(y= "% Change in Opioid Death Rate", x="    2013-2014                                                2014-2015", title="% Change in Opioid Death Rates ") + ylim(-70,150) 
dev.off()
```
...
deathsranges <- ggplot(longset) + 
        ...
        geom_errorbar(data = my_info, aes(x = region, y = sd, ymin = mean - sd, ymax = mean + sd), 
...
unique(df) %>% ggplot(aes(x=reorder(name,Deaths),y=Deaths, fill=Year)) + 
  geom_col(col="white", alpha=0.8) + labs(title="Opioid Deaths 2013 - 2015", x="State", y="Deaths") + 
...
alluvial(alluv[1:3], freq=alluv$n, alpha=0.68, xw=0.2,cex.axis=0.8, cex = 0.6, blocks=T, border="white",col = ifelse(alluv$Change.13.14 == "Significant", "maroon", "grey"))
dev.off()
```
